...
export default function AboutSection() {
    return (
        <>
            ...
            <section className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 grid grid-cols-1 md:grid-cols-10 gap-6 sm:gap-12 text-ghost_white-500">

                {/* Profielfoto */}
                <div className="sm:block md:col-span-3 order-2 md:order-1">
                    <div className="bg-cool_gray-300 rounded-2xl shadow-lg p-4 sm:p-6 flex items-center justify-center transition-all duration-300">
                        <Image
                            src="/images/pose.svg"
                            alt="Jordy Breur"
                            width={200}
                            height={200}
                            className="object-contain w-full h-auto max-w-[220px] sm:max-w-[260px]"
                            loading="lazy"
                ...
                {/* Profielbeschrijving */}
                <Card className="md:col-span-7 order-1 md:order-2 bg-cool_gray-300 text-ghost_white-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-5 sm:p-8">
            ...
            <section id="over-mij" className="py-12 sm:py-20 bg-cool_gray-200 text-ghost_white overflow-x-hidden">
                <div className="container mx-auto px-4 sm:px-6">
                    <h2 className="text-3xl sm:text-4xl font-bold text-left mb-6 sm:mb-10 text-ghost_white sm:text-center">Carriére</h2>
                    ...
                    <Carousel
                        ...
                        className="w-full"
                    >
                        <CarouselContent className="space-x-4 sm:space-x-6 scroll-smooth snap-x snap-mandatory">
                            {journeyTimeline.map((item, idx) => (
                                <CarouselItem key={idx} className="basis-72 sm:basis-80 md:basis-96 snap-center">
                                    <Card className="bg-cool_gray-300 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 h-full">
                                        <CardHeader className="pb-2 sm:pb-3">
                                            <div className="flex items-center justify-between">
                                                <CardTitle className=" text-ghost_white-800 text-lg sm:text-xl font-bold ml-4 mt-4">
                                                    {item.year}
                                                </CardTitle>
                                        ...
                                        <CardContent className="space-y-2 sm:space-y-3 p-4">
                                            <h3 className="text-lg sm:text-xl font-semibold text-ghost_white">
                                                {item.title}
                                            </h3>
                                            <p className="text-xs sm:text-sm font-semibold text-dark_spring_green-700">
                                                {item.company}
                                            </p>
                                            <p className="text-cool_gray-900 leading-relaxed text-sm">
                                                {item.description}
                                            </p>
                                        </CardContent>
                                    </Card>
                        ...
                        <CarouselPrevious className="hidden sm:inline-flex bg-cool_gray-300 border-none hover:bg-cool_gray-400 text-ghost_white" />
                        <CarouselNext className="hidden sm:inline-flex bg-cool_gray-300 border-none hover:bg-cool_gray-400 text-ghost_white" />
                    </Carousel>
                </div>
            </section>
        </>
